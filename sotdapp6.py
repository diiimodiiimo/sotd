...
st.markdown(
    ...
    <style>
        :root {
            --primary: #E6B800; /* warm soft gold */
            --bg: #000000;       /* black */
            --card: #1e1e1e;     /* dark gray */
            --warn-bg: #5c1d1d;  /* deep red */
            --warn-text: #ffffff;
        }
        ...
        .stWarning {
            ...
            border-left: 0.5rem solid #ff4d4d !important;
        }
    </style>
    """,
...
CSV_PATH = "sotd_dff.csv"

if os.path.exists(CSV_PATH):
    sotd_df_stream = pd.read_csv(CSV_PATH, dtype=str)
    sotd_df_stream["Date"] = pd.to_datetime(sotd_df_stream["Date"], errors="coerce").dt.strftime("%Y-%m-%d")
else:
    sotd_df_stream = pd.DataFrame(columns=[
        "Date", "Song Title", "Artist", "Album Title", "Album Art", "Notes",
        "Duration (ms)", "Explicit", "Popularity", "Release Date", "ID", "Track ID", "URI"
    ])

# **Define Semester Mapping Function**
def assign_semester(date):
    """Assigns a semester while ensuring the year is included."""
    year = date.year
    month = date.month

    if 1 <= month <= 5:
        return f"{year} - Jan–May"
    elif 6 <= month <= 8:
        return f"{year} - June–Aug"
    else:
        return f"{year} - Aug–Dec"

# Apply semester mapping
...
with tabs[0]:
    ...
    if date:
        ...
        if not existing_entry.empty:
            song_title = existing_entry.iloc[0]["Song Title"] if pd.notna(existing_entry.iloc[0]["Song Title"]) else "No Song Added"
            artist = existing_entry.iloc[0]["Artist"] if pd.notna(existing_entry.iloc[0]["Artist"]) else "Unknown Artist"
            st.success(f"✅ A song already exists for {date_str}: **{song_title}** by **{artist}**")

            # **Overwrite Option**
            if st.button("Would you like to overwrite it?"):
                # **Remove the existing entry from the DataFrame**
                sotd_df_stream = sotd_df_stream[sotd_df_stream["Date"] != date_str]  
                sotd_df_stream.to_csv(CSV_PATH, index=False)  # Save the change
                st.warning(f"⚠️ The existing entry for {date_str} has been removed. You can now add a new song.")

        else:
            st.warning(f"🔴 No song exists for {date_str}. Please add a song.")
                ...
                if tracks:
                    ...
                    if selected_track:
                        ...
                        if st.button("Check if in Playlist"):
                            with st.spinner("Checking if song is in playlist..."):
                            ...
                            if st.button("Add to Playlist"):
                                 ...
                                 st.success("🎶 Song successfully added to SOTD playlist!")
                        ...
                        if st.button("Save to SOTD"):
                            ...
                            sotd_df_stream = pd.concat([sotd_df_stream, pd.DataFrame([track_details])], ignore_index=True)
                            sotd_df_stream.to_csv(CSV_PATH, index=False)
